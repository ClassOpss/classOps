...
const schema = z.object({
  ...
  quizPrepLeadDays: z.coerce.number().int().min(0).max(60),
  quizAnnounceLeadDays: z.coerce.number().int().min(0).max(60),
...
export async function updateOperationConfig(_prev: ConfigState, formData: FormData): Promise<ConfigState> {
  ...
  const parsed = schema.safeParse({
    ...
    quizPrepLeadDays: String(formData.get("quizPrepLeadDays") ?? ""),
    quizAnnounceLeadDays: String(formData.get("quizAnnounceLeadDays") ?? ""),
